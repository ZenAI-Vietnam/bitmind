...
from dataset import TrainDataset, FixValDataset, RandomValDataset
...
def create_model(model_name='convnextv2_tiny.fcmae_ft_in22k_in1k_384', device=None):
    ...
    model = Model(model_name, num_classes=2)
...
def create_datasets(model, train_weights=None):
    ...
    train_dataset_file = 'all_first_16_3_classes_overfit.jsonl'
    val_dataset_file = 'val_first_16_3_classes_overfit.jsonl'
    
    train_dataset = TrainDataset(
        dataset_file=train_dataset_file, 
        base_transforms=model.get_transforms(is_training=False),
...
def validate_epoch(model, fix_val_loader, random_val_loader, 
    ...
    if rank == 0:
        for source in fix_val_acc_by_source.keys():
            ...
            weight = int((1 - max(norm_acc.item(), 0.8)) // 0.025 + 1)
...
def main():
    ...
    # ckpt_path = "/mnt/video/checkpoints_pt/bitmind_video_image_model/eva02_base_patch14_448.mim_in22k_ft_in22k_in1k_1225_d862ccd0/06.pt"
    ...
    outdir = f"checkpoints_pt/bitmind_video_image_model/{_id}"
    if rank == 0:
